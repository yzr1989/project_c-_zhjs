...
//	const int light_on_value = 110;
//	const int light_off_value = 180;
	LOG_INFO<<"size="<<size;
	for (int i=0;i<size;i++){
		...
		if (PTroominfo->harddevice_list[i]->hdtype == "1000"){
			...
			if (PTroominfo->op_name == "auto" ){

				LOG_INFO  << "auto";
				LOG_INFO<<"hd_light="<<PTroominfo->hd_light;

				/*   小于阈值 开灯 */
				if (  ( PTroominfo->hdmode == 1)  &&  (PTroominfo->hd_light  < PTroominfo->light_on ) ){
					LOG_INFO  << "on op";
					op_relay_obj(PTroominfo,i,ptr_thread_object,OP_ON);
				}
				...
				if (  ( PTroominfo->hdmode == 0)  &&  (PTroominfo->hd_light  > PTroominfo->light_off ) ){
					LOG_INFO  << "on off";
					op_relay_obj(PTroominfo,i,ptr_thread_object,OP_OFF);
				}
...
static  int    get_contextmodeldetail_info(Connection  conn ,shared_ptr<roominfo>  PTroominfo , int modelid)
{
	...
	sql_cmd  =   "select  detailid , bgtime ,edtime ,ctrltype  ,light_on , light_off  from  contextmodeldetail  where modelid="+lexical_cast<string>(modelid);
	...
	try {
		...
		for (it = res.begin(); it != res.end(); ++it) {
			cout << '\t' << it->detailid<<"\t"<<it->bgtime<<"\t"<<it->edtime<<"\t"<<it->ctrltype<<"\t"<< it->light_on<<"\t" << it->light_off<<"\t"<<endl;
			...
			try{
				PTroominfo->light_on = lexical_cast<int>( it->light_on);
			}catch (bad_lexical_cast &e  ){			
					LOG_ERROR<<"error:"<<e.what();
					PTroominfo->light_on = light_on_value;
			}

			try{
				PTroominfo->light_off  = lexical_cast<int>( it->light_off);
			}catch (bad_lexical_cast &e  ){			
					LOG_ERROR<<"error:"<<e.what();
					PTroominfo->light_off = light_off_value;
			}
...
static int handle_ggzm_op(Connection  conn ,shared_ptr<roominfo>  PTroominfo,
				shared_ptr<thread_object>  ptr_thread_object)
{
 ...
 static   int new_human_induction_hardtype_op(shared_ptr<harddevice> Ptrhdharddevice,shared_ptr<roominfo> PTroominfo, Connection  conn )
{
	...
	string sql_cmd =  "select hd_light   from harddevice   where hdid="+ lexical_cast<string>(Ptrhdharddevice->hdid);
	LOG_INFO<<"sql_cmd ="<<sql_cmd;
	try {
	...
	if (UseQueryResult res = query.use()) {
			...
			// Get each row in result set, and print its contents
		while (Row row = res.fetch_row()) {
			LOG_TRACE<<"hd_light"<<string(row["hd_light"]);
			
			try{
			 PTroominfo->hd_light = lexical_cast<float >( row["hd_light"]);
			}catch (bad_lexical_cast & e) {		
				LOG_ERROR<<"error:"<<e.what();
				PTroominfo->hd_light  =0;
			}
				...
				shared_ptr<thread_object>  ptr_thread_object)
{
	...
	handle_ggzm_op(conn,PTroominfo,ptr_thread_object);
...
int  op_roominfo_ggzm(Connection  conn,shared_ptr<thread_object>  ptr_thread_object)
{
